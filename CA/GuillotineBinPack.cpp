	...
	while(rects.size() > 0)
	{
        ...
        for(size_t i = 0; i < freeRectangles.size(); ++i)
        {
            for(size_t j = 0; j < rects.size(); ++j)
            {
                ...
                else if (rects[j].rotate && (rects[j].height == freeRectangles[i].width && rects[j].width == freeRectangles[i].height))
                {
                    bestFreeRect = i;
                    bestRect = j;
                    bestFlipped = true;
        ...
        newNode.barcodeNo = rects[bestRect].barcodeNo;
        newNode.name = rects[bestRect].name;
        newNode.rotate = rects[bestRect].rotate;

        if (bestFlipped)
        {
            std::swap(newNode.width, newNode.height);
            if (rects[bestRect].bRotated)
            {
        ...
        {
            newNode.bRotated = rects[bestRect].bRotated;
        }
...
Rect GuillotineBinPack::Insert(const RectSize& rcSz, bool merge, FreeRectChoiceHeuristic rectChoice,
	GuillotineSplitHeuristic splitMethod)
{
	// Find where to put the new rectangle.
	int freeNodeIndex = 0;
    Rect newRect = FindPositionForNewNode(rcSz, rectChoice, &freeNodeIndex);
...
Rect GuillotineBinPack::FindPositionForNewNode(const RectSize& rcSz, FreeRectChoiceHeuristic rectChoice, int *nodeIndex)
{
	Rect bestNode;
	...
	for(size_t i = 0; i < freeRectangles.size(); ++i)
	{
        ...
        if (rcSz.width == freeRectangles[i].width && rcSz.height == freeRectangles[i].height)
		{
        ...
        else if (rcSz.rotate && rcSz.height == freeRectangles[i].width && rcSz.width == freeRectangles[i].height)
		{
			bestNode.x = freeRectangles[i].x;
			bestNode.y = freeRectangles[i].y;
        ...
        else if (rcSz.width <= freeRectangles[i].width && rcSz.height <= freeRectangles[i].height)
		{
            int score = ScoreByHeuristic(rcSz.width, rcSz.height, freeRectangles[i], rectChoice);
		...
		{
            int score = ScoreByHeuristic(rcSz.height, rcSz.width, freeRectangles[i], rectChoice);
