...
class CASHARED_EXPORT SkylineBinPack
{
public:
    ...
    Rect Insert(const RectSize& rcSz, LevelChoiceHeuristic method);
...
private:
    ...
    Rect InsertBottomLeft(const RectSize& rcSz);
    Rect InsertMinWaste(const RectSize& rcSz);

    Rect FindPositionForNewNodeMinWaste(const RectSize& rcSz, int &bestHeight, int &bestWastedArea, int &bestIndex) const;
    Rect FindPositionForNewNodeBottomLeft(const RectSize& rcSz, int &bestHeight, int &bestWidth, int &bestIndex) const;
